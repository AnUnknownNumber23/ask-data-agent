...
**v2.0 更新：**
- 多轮 ReAct 循环（CHECK 节点替代 ATTRIBUTE/PREDICT）
- LLM 判断非数据问题并拒绝
- embedding 升级为 multilingual-MiniLM-L12-v2
- 错误信息附带调整建议
- 100 条基准 100% 通过，91 条测试 0 失败
...
**关键变化（v1 → v2）：**
- ATTRIBUTE 和 PREDICT 节点移除，归因和预测融入 CHECK→REASON 多轮循环
- CHECK 判断"问题回答完了吗"，没回答完自动下钻/外推
- UNDERSTAND 增加 `is_data_question` 字段，LLM 自行判断
- REFLECT 修正后直接跑（跳过 REASON），避免 LLM 重新生成错误的 SQL
